...
#Change sex at birth to be 0 male, 1 female
tab1(lsacwide$zf02m1, cum.percent = TRUE) # Study child sex, 1 male; 2 female
lsacwide$sex_at_birth <- ifelse(lsacwide$zf02m1 ==1, 0,
                           ifelse(lsacwide$zf02m1 == 2, 1, NA))

tab1(lsacwide$sex_at_birth, cum.percent = TRUE)

# Create new variable gender_imputed
lsacwide$gender_imputed <- ifelse(
  is.na(lsacwide$gender),  # Check if gender is NA
  lsacwide$sex_at_birth,   # If NA, use sex_at_birth (0 or 1)
  lsacwide$gender          # If not NA, keep gender (0, 1, or 2)
)

tab1(lsacwide$gender_imputed)

#Create dummy variables
lsacwide$transgender <- ifelse(lsacwide$gender_imputed ==2, 1, 0)
tab1(lsacwide$transgender, cum.percent = TRUE)

lsacwide$female <- ifelse(lsacwide$gender_imputed ==1, 1, 0)
tab1(lsacwide$female, cum.percent = TRUE)

lsacwide$male <- ifelse(lsacwide$gender_imputed ==0, 1, 0)
tab1(lsacwide$male, cum.percent = TRUE)
...
# Parent 1 SUD
lsacwide$p1_dud <- ifelse(lsacwide$gp1_dud == 1 | lsacwide$ip1_dud == 1, 1, 0)
tab1(lsacwide$p1_dud, cum.percent = TRUE)
...
# Missingness of iDV and anyDV above 50%
# Remove iDV
lsacwide$domestic_violence <- ifelse(lsacwide$cDV == 1 | lsacwide$dDV == 1 | lsacwide$eDV == 1 | 
                           lsacwide$fDV == 1 | lsacwide$gDV == 1 | lsacwide$hDV == 1, 1, 0)

tab1(lsacwide$domestic_violence)

# Create a new variable indicating whether participant has available data on 5 out of 6 variables
lsacwide$available_DV <- rowSums(!is.na(lsacwide[c("cDV", "dDV", "eDV", "fDV", "gDV", "hDV")])) >= 5

# Calculate score for participants with available data on  out of 6 variables
lsacwide$nwaves_DV <- ifelse(lsacwide$available_DV,
                                  rowSums(lsacwide[c("cDV", "dDV", "eDV", "fDV", "gDV", "hDV")], na.rm = TRUE),
                                  NA)

tab1(lsacwide$nwaves_DV, cum.percent = TRUE)
lsacwide$any_DV <- ifelse(lsacwide$nwaves_DV %in% 1:6, 1,
                           ifelse(lsacwide$nwaves_DV ==0, 0, NA))

tab1(lsacwide$any_DV, cum.percent = TRUE)
...
lsac_analysis <- lsacwide[, c("transgender", "male", "female", "csep", "ccnfsad", 
                              ...
                              "homes_since_birth",
                                ...
                                "PAE", "parent1_MHdisorder", 
                                "p1_alcp", "p1_dud","cak6s", "dak6s", "eak6s", "fak6s",
                                ...
                                "close_dad12", "close_dad14", "max_psupport", 
